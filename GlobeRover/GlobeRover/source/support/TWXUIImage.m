...
-(UIImage*)resizedImageToSize:(CGSize)dstSize
{
	...
	switch(orient) {
		...
		case UIImageOrientationLeftMirrored: //EXIF = 5
			...
			transform = CGAffineTransformScale(transform, -1.0, 1.0);
			transform = CGAffineTransformRotate(transform, 3.f * (CGFloat)M_PI_2);
			break;  
...
+ (UIImage *)retinaImageWithContentsOfFile:(NSString *)path
{
    ...
    if (fileData)
    {
        if ([UIImage respondsToSelector:@selector(imageWithData:scale:)])
            result = [UIImage imageWithData:fileData scale:2.0];
        else // < iOS 6.0
            result = [UIImage.alloc initWithCGImage:[UIImage imageWithData:fileData].CGImage scale:2.0 orientation:UIImageOrientationUp];
    }
    return result;
}
